...
<div id="body">
    ...
    <div class="rcontent">
	  ...
	  if(!strcmp(strtolower($_GET['name']),"product") & isset($_GET['success'])){
				$result=$connect->query("UPDATE product SET product_name='{$_POST['product_name']}',cost_price={$_POST['cprice']},supplier_id={$_POST['supplier']},quantity={$_POST['quantity']},product_type='{$_POST['product_type']}',market_price={$_POST['mprice']} WHERE product_id='{$_POST['product_id']}'");
	   ...
	   if(isset($_GET['name'])&isset($_GET['id'])){
		...
		if(!strcmp(strtolower($_GET['name']),"product")){
			...
			echo"<div id='data'>";
				...
				$plist=$connect->query("SELECT * FROM product WHERE product_id='{$_GET['id']}'");
				$plist=$plist->fetch_array();
			echo "<form method='post' action='editlist.php?name=product&success=1'>
					  <table>
						...
						$dept_set = $connect->query("SELECT dept_id, dept_name FROM department");
				while($row = $dept_set->fetch_array($dept_set))
					...
					echo"</select>
						...
						$supplier_set = $connect->query("SELECT sid, sname FROM supplier");
				while($row = $supplier_set->fetch_array())
	  ...
	  //supplier start
	  	  if(!strcmp(strtolower($_GET['name']),"supplier") & isset($_GET['success'])){
				$result=$connect->query("UPDATE supplier SET sname='{$_POST['name']}',saddress='{$_POST['address']}',sdealer='{$_POST['dealer']}',sphone={$_POST['phone']},semail='{$_POST['email']}' WHERE sid='{$_POST['sid']}'");
				if(!$result)echo "Addition not successful ".mysql_error();
	   ...
	   if(isset($_GET['name'])&isset($_GET['id'])){
		...
		if(!strcmp(strtolower($_GET['name']),"supplier")){
			...
			echo"<div id='data'>";
				...
				$plist=$connect->query("select * from supplier where sid='{$_GET['id']}'");
				$plist = $plist->fetch_array();
	  ...
	  //customer start
	  	  if(!strcmp(strtolower($_GET['name']),"customer") & isset($_GET['success'])){
				$result=$connect->query("UPDATE customer SET first_name='{$_POST['fname']}',last_name='{$_POST['lname']}',caddress='{$_POST['caddress']}',cphone={$_POST['cphone']} WHERE cid='{$_POST['cid']}'");
				if(!$result)echo "Addition not successful ".mysql_error();
	   ...
	   if(isset($_GET['name'])&isset($_GET['id'])){
		...
		if(!strcmp(strtolower($_GET['name']),"customer")){
			...
			echo"<div id='data'>";
				...
				$plist=$connect->query("select * from customer where cid='{$_GET['id']}'");
				$plist=$plist->fetch_array();
